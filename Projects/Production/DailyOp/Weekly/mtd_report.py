...
if __name__ == '__main__':
    ...
    d0 = dt.date(dn.year, dn.month, 1)
    contexts = ['changeling', 'cyclops', 'havok', 'sway', 'cannonball', 'colossus', 'husk', 'cecilia', 'sunfire']
    plot_folder = '/Volumes/Tamedog_2T/AirPort_Work/Trading/projects/AirPort_Xman/data/production/monthly/'
    live_plot_path = os.path.join(plot_folder, 'live/monthly_live_{}.png'.format(dn))
    live_sim_plot_path = os.path.join(plot_folder, 'live_sim/monthly_live_sim_{}.png'.format(dn))
    live_exch_plot_path = os.path.join(plot_folder, 'live_exch/monthly_live_exch_{}.png'.format(dn))
    ...
    msg = MIMEMultipart()
    msg['subject'] = '[Xman] monthly report: {} - {}'.format(d0, dn)
    msg['From'] = from_email
