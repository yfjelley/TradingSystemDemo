...
def handle_data(context, data):
	# set context.can_buy = True, context.can_sell = False in initialize()
	bid_price = 999999
	ask_price = 0
	quantity = 100
	...
	if context.can_buy:
		if context.portfolio.positions[context.asset].amount < 1000:
			order(context.asset, quantity, limit_price = bid_price)
		else:
			context.can_sell = True
			context.can_buy = False
	if context.can_sell:
		if context.portfolio.positions[context.asset].amount > 0:
			order(context.asset, -1 * quantity, limit_price = ask_price)
		else:
			return

def analyze(context, perf):
	print(context.perf_tracker.todays_performance.processed_transactions)
...
if __name__ == '__main__':
	...
	cmx_config = {
				...
				'time_end': pd.to_datetime('2018-8-16', utc=True),
