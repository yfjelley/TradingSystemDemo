...
class leg_order:
    def __init__(self, context, leg_index):
        ...
        self.asset = self.context.cmx_config.catalyst_assets[leg_index]
        self.symbol = self.asset.symbol
        self.ticksize = self.context.cmx_config.invent_ticksizes[leg_index]
        self.is_live = self.context.cmx_config.global_mode == 'live'
        self.ignore_parital_fill = self.context.cmx_config.invent_ignore_partial_fills[leg_index]
        self.min_share = self.context.cmx_config.invent_min_shares[leg_index]
    ...
    def reload_config(self):
        self.asset = self.context.cmx_config.catalyst_assets[self.leg_index]
        self.symbol = self.asset.symbol
        self.ticksize = self.context.cmx_config.invent_ticksizes[self.leg_index]
        self.is_live = self.context.cmx_config.global_mode == 'live'
        self.ignore_parital_fill = self.context.cmx_config.invent_ignore_partial_fills[self.leg_index]
        self.min_share = self.context.cmx_config.invent_min_shares[self.leg_index]

    def _update(self):
        context_orders  = self.context.blotter.open_orders[self.asset]
        ...
        # if self.is_live:
        #   exchange_orders = get_open_orders(asset)
    ...
    def _get_decimal_price(self, price):
        if price is None:
            return None
        return Decimal(price).quantize(Decimal(str(self.ticksize)))
    ...
    def _update_order(self, price, qty, order_side):
        qty = int(qty / self.ignore_parital_fill) * self.ignore_parital_fill
        updated_all = True
        cum_qty = 0
        remained_orders = []
        if order_side == side.buy:
            ...
            for o in ts_based_orders:
                ...
                if o.filled != 0 and o.amount != o.filled:
                    ...
                    cum_qty += o.amount
                    continue
                if cum_qty + o.amount >= qty + self.min_share / float(price):
                    # from this order and forward, we need to cancel:
                    cancelled = self._cancel_orders([o])
        ...
        if cum_qty != qty:
            if abs(qty - cum_qty) >= self.min_share / float(price):
                self._split_order(qty - cum_qty, limit_price = float(price))
                ...
                for o in remained_orders[::-1]:
                    ...
                    if cancelled:
                        ...
                        if abs(new_qty) >= self.min_share / price:
                            break
                if abs(new_qty) >= self.min_share / float(price):
                    self._split_order(new_qty, limit_price = float(price))
    ...
    def _split_order(self, qty, limit_price):
        min_qty = self.min_share / limit_price
        residual_qty = qty
        if qty > 0:
            while residual_qty >= 2 * min_qty:
                order(self.asset, min_qty, limit_price)
                residual_qty -= min_qty
            order(self.asset, residual_qty, limit_price)
        elif qty < 0:
            while residual_qty <= -2 * min_qty:
                order(self.asset, -1 * min_qty, limit_price)
                residual_qty += min_qty
            order(self.asset, residual_qty, limit_price)
    ...
    def send_orders(self, price_qty_map, order_side):
        ...
        if price_qty_map is None or len(price_qty_map) == 0:
            ...
            if order_side == side.buy:
                ...
                for p, ord_ls in self.bid_map.items():
                    ...
                    updated_all &= self._cancel_orders(ord_ls)
                if updated_all:
        ...
        if order_side == side.buy:
            ...
            for p, ord_ls in self.bid_map.items():
                if p not in decimal_price_qty_map:
                    ...
                    if decimal_price_qty_map[p] > 0:
                        ...
                        updated_all &= self._update_order(p, decimal_price_qty_map[p], order_side)
            ...
            for p, q in decimal_price_qty_map.items():
                if p is None:
                    continue
                if not price_update_map[p] and q >= self.min_share / float(p):
                    self._split_order(q, limit_price = float(p))
        ...
        if order_side == side.sell:
            ...
            # if updated_all or not self.is_live:
            for p, q in decimal_price_qty_map.items():
                if not price_update_map[p] and q <= -1 * self.min_share / float(p):
                    self._split_order(q, limit_price = float(p))
                    logging.info('[cmx_order][send_orders] send ask {} * {}'.format(p, q))
            return updated_all


class multileg_orders:
    def __init__(self, context):
        self.context = context
        self.leg_num = context.cmx_config.global_leg_num
        self.leg_orders = [leg_order(context, i) for i in range(self.leg_num)]
        self.assets = context.cmx_config.catalyst_assets
        self.symbols = context.cmx_config.global_symbols

    def reload_config(self):
        for lo in self.leg_orders:
            lo.reload_config()

    def send_orders(self, asset, price_qty_map, order_side):
        try:
            leg_index = self.symbols.index(asset.symbol)
            self.leg_orders[leg_index].send_orders(price_qty_map, order_side)
        except:
            pass
